...
interface DeleteShowcaseProductModalProps {
  isOpen: boolean;
  closeModal: () => void;
  showcase: ShowcaseProductType | null;
  onConfirm: (id: string) => Promise<void>;
  isLoading?: boolean;
}

export function DeleteShowcaseProductModal({
  isOpen,
  closeModal,
  showcase: selectedShowcase,
  onConfirm,
  isLoading = false,
}: DeleteShowcaseProductModalProps) {
  const handleConfirm = async () => {
    if (!selectedShowcase?._id) return;

    try {
      await onConfirm(selectedShowcase._id);
      closeModal();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl">
        ...
        <div className="p-6">
          <p className="text-sm text-gray-600 mb-4">
            ...
            <span className="font-medium">{selectedShowcase.showcaseType}</span>
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-1">
                  This will permanently delete the showcase and all associated
                  product references.
                </p>
              </div>
            </div>

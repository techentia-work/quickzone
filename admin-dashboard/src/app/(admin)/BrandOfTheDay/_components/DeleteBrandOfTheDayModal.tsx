...
import { AlertTriangle, Trash2, X, BadgeCheck } from "lucide-react";
import Image from "next/image";
import { BrandOfTheDayType } from "@/lib/types/brandOfTheDay/brandOfTheDay.types";

interface DeleteBrandOfTheDayModalProps {
  isOpen: boolean;
  closeModal: () => void;
  item: BrandOfTheDayType | null;
  onConfirm: (id: string) => Promise<void>;
  isLoading?: boolean;
}

export function DeleteBrandOfTheDayModal({
  isOpen,
  closeModal,
  item: selectedItem,
  onConfirm,
  isLoading = false,
}: DeleteBrandOfTheDayModalProps) {
  const handleConfirm = async () => {
    ...
    try {
      ...
      console.error("Error deleting brand of the day:", error);
    }
  };
          ...
          <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-md p-3 mb-4">
            {selectedItem.banner ? (
              ...
              <div className="w-14 h-14 bg-gray-200 rounded-md flex items-center justify-center">
                <BadgeCheck className="w-6 h-6 text-gray-500" />
              </div>
            )}
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-1">
                  This will permanently remove the brand from the
                  <strong> Brand Of The Day </strong>
                  section on homepage/app. The original brand website
                  or data will not be affected.
                </p>
              </div>
            </div>
          </div>

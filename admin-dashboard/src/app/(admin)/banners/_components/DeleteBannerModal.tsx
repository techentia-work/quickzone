...
export function DeleteBannerModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onConfirm(selectedBanner._id);
      closeModal();
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-xl">
        ...
        <div className="p-6">
          <p className="text-sm text-gray-600 mb-4">
            ...
            <span className="font-medium text-gray-900">"{selectedBanner.title}"</span>?
          ...
          <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-md p-3 mb-4">
            ...
            <div>
              <span className="text-sm font-semibold text-gray-800">{selectedBanner.title}</span>
              {selectedBanner.category && (
                <p className="text-xs text-gray-500">
                  Category: {selectedBanner.category?.name || "-"}
                </p>
              )}
              {selectedBanner.subcategory && (
                <p className="text-xs text-gray-500">
                  Subcategory: {selectedBanner.subcategory?.name || "-"}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="flex">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 mt-1">
                  This will permanently delete the banner and may affect your homepage layout.
                </p>
              </div>
            </div>
          </div>

...
export function AddShopByStoreForm() {
  const {
    formData,
    errors,
    bannerFile,
  ...
  const { create } = useAdminShopByStore();

  // ✅ get ALL categories
  const { adminCategories } = useAdminCategory();

  // ✅ ONLY MASTER categories
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Shop by store created successfully");
      resetForm();
    } catch (err) {
      console.error(err);
      toast.error("Error creating shop by store");
    } finally {
